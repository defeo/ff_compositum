...
(see~\cite[Ch.~IX.1.8]{BourbakiAlgCom9}). The added level abstraction
...
Next, we discuss some classical facts about the trace from, and give
algorithms to change between monomial bases and their duals. In all
this section, $k$ is a perfect field. General reference for the
...
written on the dual bases, the following algorithm requires all $b_i$
...
\end{eqnarray*}
with $h_\ell(z)=\sum_{0 \le i < m} b_{i,i+\ell-m+1} z^i$ for all
$\ell$ (undefined indices are set to zero). Hence $a=\Phi(b)$ has the
form
...
We define the matrix $M_H=[h_{iq+j}]_{0 \le i <p, 0 \le j < q}$
containing the polynomials $h_\ell$ organized in a row-major fashion,
and compute the product $M_V=M_H M_T$. We can then construct
